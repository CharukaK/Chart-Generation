...
class ChartGenerator extends React.Component {
    ...
    generatePieChart(config, data) {
        return (
            <PieChart width={this.state.width} height={this.state.height}>
                ...
                >
                    {
                        data.pieData.map((entry, index) => <Cell key={`pieChart-${index}`}
                ...
                <tr>
                    {
                        config.columns.map((column, i) => {
                            return (<th key={`tableHeader-${i}`} style={{
    ...
    mapGenerator(mapConfig, data) {
        ...
        return (
            <Map width={this.state.width} height={this.state.height} config={trans} legend={mapConfig.charts.colors}>

                <Topographies inbuiltMapType="europe">
                    {(topographies, projection) => topographies.map((topography, i) => {
                        console.log(data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id));
                        return (
                            <Topography
                                key={i}
                                ...
                                style={{
                                    ...
                                    default: {
                                        ...
                                        fill: data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id).length > 0 ?
                                            this.mapColorScale(mapConfig.charts.colors.domain, mapConfig.charts.colors.range)(data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id)[0][mapConfig.charts.y]) : '#607D8B',
                                        stroke: '#607D8B',
                                        strokeWidth: 0.75,
                                        outline: 'none',
                                    },
                                ...
                                onClick={() => {
                                    console.info(topography);
                                }}
                                tooltip={
                                    data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id).length > 0 ?
                                        `${mapConfig.charts.x}: ${data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id)[0][mapConfig.charts.x]}, ${mapConfig.charts.y}: ${data.filter(e => this.convertCountryNamesToCode(e[mapConfig.charts.x]) === topography.id)[0][mapConfig.charts.y]}`:
                                        ''
                                }
                            />
                        );
